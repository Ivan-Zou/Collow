...
const county_metrics_by_date = async function(req, res) {
  ...
  connection.query(`
  SELECT C.id, C.name AS Name, LP.average AS Average, LP.median AS Median, LC.active AS Active, LC.total AS Total, 
  SF.median_listing_price_per_square_foot AS Square_Price, SF.median_square_feet AS Square_Feet
  FROM County C JOIN Listing_Price LP ON C.id = LP.id
                JOIN Listing_Count LC ON LP.id = LC.id AND LP.date = LC.date 
                JOIN Square_Footage SF ON LC.id = SF.id AND LC.date = SF.date
